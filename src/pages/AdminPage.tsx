...
import { WartungsvertraegeViewDialog } from '@/components/dialogs/WartungsvertraegeViewDialog';
...
const WARTUNGSVERTRAEGE_FIELDS = [
  ...
  { key: 'anlagentyp', label: 'Anlagentyp', type: 'lookup/select', options: [{ key: 'gasheizung', label: 'Gasheizung' }, { key: 'oelheizung', label: 'Ölheizung' }, { key: 'waermepumpe', label: 'Wärmepumpe' }, { key: 'pelletheizung', label: 'Pelletheizung' }, { key: 'fernwaerme', label: 'Fernwärme' }, { key: 'sonstiges', label: 'Sonstiges' }] },
  { key: 'anlagenstandort_strasse', label: 'Anlagenstandort – Straße', type: 'string/text' },
  { key: 'anlagenstandort_hausnummer', label: 'Anlagenstandort – Hausnummer', type: 'string/text' },
  { key: 'anlagenstandort_plz', label: 'Anlagenstandort – Postleitzahl', type: 'string/text' },
  { key: 'anlagenstandort_ort', label: 'Anlagenstandort – Ort', type: 'string/text' },
  { key: 'wartungsintervall', label: 'Wartungsintervall', type: 'lookup/radio', options: [{ key: 'jaehrlich', label: 'Jährlich' }, { key: 'halbjaehrlich', label: 'Halbjährlich' }] },
  { key: 'letzte_wartung', label: 'Datum der letzten Wartung', type: 'date/date' },
  { key: 'naechste_wartung', label: 'Nächste Wartung fällig am', type: 'date/date' },
  { key: 'wartungsstatus', label: 'Wartungsstatus', type: 'lookup/select', options: [{ key: 'geplant', label: 'Geplant' }, { key: 'durchgefuehrt', label: 'Durchgeführt' }, { key: 'ueberfaellig', label: 'Überfällig' }] },
  { key: 'jahreswert', label: 'Jährlicher Vertragswert (€)', type: 'number' },
...
const ENTITY_TABS = [
  ...
  { key: 'wartungsvertraege', label: 'Wartungsverträge', pascal: 'Wartungsvertraege' },
] as const;
...
export default function AdminPage() {
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      ...
      case 'wartungsvertraege': return (data as any).wartungsvertraege as Wartungsvertraege[] ?? [];
      default: return [];
    }
  }, [data]);
  ...
  const getApplookupDisplay = useCallback((entity: EntityKey, fieldKey: string, url?: unknown) => {
    ...
    if (entity === 'wartungsvertraege' && fieldKey === 'kunde') {
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    switch (entity) {
      ...
      case 'wartungsvertraege': return WARTUNGSVERTRAEGE_FIELDS;
      default: return [];
    }
  }, []);
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      ...
      case 'wartungsvertraege': return {
        create: (fields: any) => LivingAppsService.createWartungsvertraegeEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateWartungsvertraegeEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteWartungsvertraegeEntry(id),
      };
      default: return null;
    }
  }, []);
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'wartungsvertraege' || dialogState?.entity === 'wartungsvertraege') && (
        <WartungsvertraegeDialog
          open={createEntity === 'wartungsvertraege' || dialogState?.entity === 'wartungsvertraege'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'wartungsvertraege' ? handleUpdate : (fields: any) => handleCreate('wartungsvertraege', fields)}
          defaultValues={dialogState?.entity === 'wartungsvertraege' ? dialogState.record?.fields : undefined}
      ...
      {viewState?.entity === 'wartungsvertraege' && (
        <WartungsvertraegeViewDialog
          open={viewState?.entity === 'wartungsvertraege'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'wartungsvertraege', record: r }); }}
          kundenverwaltungList={(data as any).kundenverwaltung ?? []}
        />
      )}
