...
import type { Kundenverwaltung, Wartungsvertraege, CreateKundenverwaltung, CreateWartungsvertraege } from '@/types/app';
...
export class LivingAppsService {
  ...
  // --- WARTUNGSVERTRAEGE ---
  static async getWartungsvertraege(): Promise<Wartungsvertraege[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.WARTUNGSVERTRAEGE}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Wartungsvertraege[];
    return enrichLookupFields(records, 'wartungsvertraege');
  }
  static async getWartungsvertraegeEntry(id: string): Promise<Wartungsvertraege | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.WARTUNGSVERTRAEGE}/records/${id}`);
    const record = { record_id: data.id, ...data } as Wartungsvertraege;
    return enrichLookupFields([record], 'wartungsvertraege')[0];
  }
  static async createWartungsvertraegeEntry(fields: CreateWartungsvertraege) {
    return callApi('POST', `/apps/${APP_IDS.WARTUNGSVERTRAEGE}/records`, { fields: cleanFieldsForApi(fields as any, 'wartungsvertraege') });
  }
  static async updateWartungsvertraegeEntry(id: string, fields: Partial<CreateWartungsvertraege>) {
    return callApi('PATCH', `/apps/${APP_IDS.WARTUNGSVERTRAEGE}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'wartungsvertraege') });
  }
  static async deleteWartungsvertraegeEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.WARTUNGSVERTRAEGE}/records/${id}`);
  }
