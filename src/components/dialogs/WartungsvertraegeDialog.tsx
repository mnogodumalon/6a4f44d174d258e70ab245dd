...
import type { Wartungsvertraege, Kundenverwaltung, LookupValue } from '@/types/app';
...
import { formEnhancements, computedDeps, computedApplookupRefs } from '@/config/form-enhancements/Wartungsvertraege';
...
interface WartungsvertraegeDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Wartungsvertraege['fields']) => Promise<void>;
  /** SHAPE-TOLERANT: lookup fields accept the bare key (string) or the
   *  LookupValue object; applookup fields the bare record id or the full
   *  record URL — the dialog normalizes both. */
  defaultValues?: Omit<Wartungsvertraege['fields'], 'anlagentyp' | 'wartungsintervall' | 'wartungsstatus'> & {
    anlagentyp?: LookupValue | string;
    wartungsintervall?: LookupValue | string;
    wartungsstatus?: LookupValue | string;
  };
...
const NORMALIZE_LOOKUPS: Record<string, readonly { key: string; label: string }[]> = {
  anlagentyp: LOOKUP_OPTIONS['wartungsvertraege']?.['anlagentyp'] ?? [],
  wartungsintervall: LOOKUP_OPTIONS['wartungsvertraege']?.['wartungsintervall'] ?? [],
  wartungsstatus: LOOKUP_OPTIONS['wartungsvertraege']?.['wartungsstatus'] ?? [],
};
...
export function WartungsvertraegeDialog({ open, onClose, onSubmit, defaultValues, recordId, kundenverwaltungList, enablePhotoScan = true, enablePhotoLocation = true }: WartungsvertraegeDialogProps) {
  const [fields, setFields] = useState<Partial<Wartungsvertraege['fields']>>({});
  ...
  const REQUIRED_FIELDS = ['kunde', 'anlagentyp', 'wartungsintervall', 'naechste_wartung', 'wartungsstatus', 'jahreswert'] as const;
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults(normalizedDefaults ?? {}, formEnhancements.defaults) as Partial<Wartungsvertraege['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'wartungsvertraege');
      await onSubmit(clean as Wartungsvertraege['fields']);
      onClose();
    } catch (err) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "kunde": string | null, // Display name from Kundenverwaltung (see <available-records>)\n  "anlagentyp": LookupValue | null, // Anlagentyp (select one key: "gasheizung" | "oelheizung" | "waermepumpe" | "pelletheizung" | "fernwaerme" | "sonstiges") mapping: gasheizung=Gasheizung, oelheizung=Ölheizung, waermepumpe=Wärmepumpe, pelletheizung=Pelletheizung, fernwaerme=Fernwärme, sonstiges=Sonstiges\n  "anlagenstandort_strasse": string | null, // Anlagenstandort – Straße\n  "anlagenstandort_hausnummer": string | null, // Anlagenstandort – Hausnummer\n  "anlagenstandort_plz": string | null, // Anlagenstandort – Postleitzahl\n  "anlagenstandort_ort": string | null, // Anlagenstandort – Ort\n  "wartungsintervall": LookupValue | null, // Wartungsintervall (select one key: "jaehrlich" | "halbjaehrlich") mapping: jaehrlich=Jährlich, halbjaehrlich=Halbjährlich\n  "letzte_wartung": string | null, // YYYY-MM-DD\n  "naechste_wartung": string | null, // YYYY-MM-DD\n  "wartungsstatus": LookupValue | null, // Wartungsstatus (select one key: "geplant" | "durchgefuehrt" | "ueberfaellig") mapping: geplant=Geplant, durchgefuehrt=Durchgeführt, ueberfaellig=Überfällig\n  "jahreswert": number | null, // Jährlicher Vertragswert (€)\n  "bemerkungen": string | null, // Bemerkungen\n}`;
      ...
      setFields(prev => {
        ...
        return merged as Partial<Wartungsvertraege['fields']>;
      });
      setAiText('');
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues ? 'Wartungsverträge bearbeiten' : 'Wartungsverträge hinzufügen';
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    ...
    'anlagentyp': (
      <div key="anlagentyp" className="space-y-1.5">
        <Label htmlFor="anlagentyp">Anlagentyp <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Select
          value={lookupKey(fields.anlagentyp) ?? ''}
          onValueChange={v => setFields(f => ({ ...f, anlagentyp: v === 'none' ? undefined : v as any }))}
        >
          <SelectTrigger id="anlagentyp" className="max-sm:h-11"><SelectValue placeholder="z. B. Gasheizung, Ölheizung" /></SelectTrigger>
          <SelectContent>
        ...
        {showErrors && !fields.anlagentyp && (
          <p className="text-xs text-destructive mt-1">Pflichtfeld</p>
        )}
      </div>
    ),
    'anlagenstandort_strasse': (
      <div key="anlagenstandort_strasse" className="space-y-1.5">
    ...
    'anlagenstandort_hausnummer': (
      <div key="anlagenstandort_hausnummer" className="space-y-1.5">
    ...
    'anlagenstandort_plz': (
      <div key="anlagenstandort_plz" className="space-y-1.5">
    ...
    'anlagenstandort_ort': (
      <div key="anlagenstandort_ort" className="space-y-1.5">
    ...
    'wartungsintervall': (
      <div key="wartungsintervall" className="space-y-1.5">
        <Label htmlFor="wartungsintervall">Wartungsintervall <span className="text-destructive" aria-hidden="true">*</span></Label>
        ...
        {showErrors && !fields.wartungsintervall && (
          <p className="text-xs text-destructive mt-1">Pflichtfeld</p>
        )}
      </div>
    ),
    'letzte_wartung': (
      <div key="letzte_wartung" className="space-y-1.5">
        <Label htmlFor="letzte_wartung">Datum der letzten Wartung</Label>
        <DatePicker
          id="letzte_wartung"
          placeholder="Wann war die Wartung?"
          mode="date"
    ...
    'naechste_wartung': (
      <div key="naechste_wartung" className="space-y-1.5">
        <Label htmlFor="naechste_wartung">Nächste Wartung fällig am <span className="text-destructive" aria-hidden="true">*</span></Label>
        <DatePicker
          id="naechste_wartung"
          placeholder="Wann ist die Wartung fällig?"
          mode="date"
          value={fields.naechste_wartung ?? null}
          onChange={v => setFields(f => ({ ...f, naechste_wartung: v ?? undefined }))}
          required
        />
        {showErrors && !fields.naechste_wartung && (
          <p className="text-xs text-destructive mt-1">Pflichtfeld</p>
        )}
      </div>
    ),
    'wartungsstatus': (
      <div key="wartungsstatus" className="space-y-1.5">
        <Label htmlFor="wartungsstatus">Wartungsstatus <span className="text-destructive" aria-hidden="true">*</span></Label>
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          <button
            type="button"
            role="radio"
        ...
        {showErrors && !fields.wartungsstatus && (
          <p className="text-xs text-destructive mt-1">Pflichtfeld</p>
        )}
      </div>
    ),
    'jahreswert': (
      <div key="jahreswert" className="space-y-1.5">
        <Label htmlFor="jahreswert">Jährlicher Vertragswert (€) <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          id="jahreswert"
          type="number"
          step="any"
          {...numberInputProps(formEnhancements, 'jahreswert')}
          placeholder="z. B. 250,00"
          value={fields.jahreswert !== undefined ? fields.jahreswert : (computedValues['jahreswert'] ?? '')}
          onChange={e => setFields(f => ({ ...f, jahreswert: clampNumberValue(formEnhancements, 'jahreswert', e.target.value) }))}
        />
        {showErrors && !fields.jahreswert && (
          <p className="text-xs text-destructive mt-1">Pflichtfeld</p>
        )}
      </div>
    ),
    'bemerkungen': (
      <div key="bemerkungen" className="space-y-1.5">
        ...
        <Textarea
          id="bemerkungen"
          placeholder="Besonderheiten, Anforderungen, Notizen..."
  ...
  const FIELD_LABELS: Record<string, string> = {"kunde": "Kunde", "anlagentyp": "Anlagentyp", "anlagenstandort_strasse": "Anlagenstandort – Straße", "anlagenstandort_hausnummer": "Anlagenstandort – Hausnummer", "anlagenstandort_plz": "Anlagenstandort – Postleitzahl", "anlagenstandort_ort": "Anlagenstandort – Ort", "wartungsintervall": "Wartungsintervall", "letzte_wartung": "Datum der letzten Wartung", "naechste_wartung": "Nächste Wartung fällig am", "wartungsstatus": "Wartungsstatus", "jahreswert": "Jährlicher Vertragswert (€)", "bemerkungen": "Bemerkungen"};
  const CURRENCY_KEYS = new Set<string>(["jahreswert"]);
  // Applookup-Referenz-Labels: pro applookup-Feld in dieser Form (ownKey)
  // eine Map { lookupKey: label } für ALLE Felder des Target-Schemas. Wird
  // beim Render-Walk gefiltert auf die in der computed-Formel tatsächlich
  // referenzierten lookupKeys (siehe applookupRefs unten).
  const APPLOOKUP_LABELS: Record<string, Record<string, string>> = {"kunde": {"vorname": "Vorname", "nachname": "Nachname", "strasse": "Straße", "hausnummer": "Hausnummer", "plz": "Postleitzahl", "ort": "Ort", "telefon": "Telefon", "email": "E-Mail"}};
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 py-4 space-y-4 min-w-0">
            ...
            {recordId && (
              <div className="pt-2 border-t border-border">
                <AttachmentsSection appId={APP_IDS.WARTUNGSVERTRAEGE} recordId={recordId} />
              </div>
            )}
          </div>
          {submitError && (
